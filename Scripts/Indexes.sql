...
CREATE INDEX IDX_DESC_DESC ON DESCRIPTIONS(BIKE_DESC) INDEXTYPE IS CTXSYS.CTXRULE;
CREATE INDEX IDX_DESC_MATERIAL ON DESCRIPTIONS(MATERIAL) INDEXTYPE IS CTXSYS.CTXRULE;
CREATE INDEX IDX_DESC_TYPE ON DESCRIPTIONS(BIKE_TYPE) INDEXTYPE IS CTXSYS.CTXRULE;
CREATE INDEX IDX_RATE_MSG ON RATINGS(RATING_MESSAGE) INDEXTYPE IS CTXSYS.CTXRULE ;

DROP INDEX IDX_ORDER_BUYER
...
BEGIN
    ...
    WHERE CONTAINS(BIKE_NAME,NAME,1) > 0
    ORDER BY B.BIKE_ID;
    RETURN B_RESULT;
END;
...
CREATE OR REPLACE FUNCTION CONTAINS_RATING_MESSAGE
(
    QUR IN VARCHAR2
)
RETURN INTEGER
...
BEGIN
    SELECT RATING_ID INTO RID FROM RATINGS WHERE CONTAINS(RATING_MESSAGE, QUR, 1) > 0;
    RETURN RID;
EXCEPTION
...
CREATE OR REPLACE FUNCTION MATCHES_TYPE
(
TEXT VARCHAR2
)
RETURN INTEGER
IS
    DID DESCRIPTIONS.DESC_ID%TYPE;
BEGIN
    SELECT DESC_ID INTO DID FROM DESCRIPTIONS WHERE MATCHES(BIKE_TYPE,TEXT) > 0;
    RETURN DID;
END;
